...
N_VISION = 1
N_INTERO = 1
N_TOTAL = N_MOTOR + N_VISION + N_INTERO
...
def main():
    ...
    train_np  = np.random.rand(48, 200, N_TOTAL)
    motor_train_np  = np.random.rand(48, 200, N_MOTOR)
    vision_train_np  = np.random.rand(48, 200, N_VISION)
    intero_train_np = np.random.rand(48, 200, N_INTERO)

    directory_list = sorted(glob.glob("./target/*.txt"))
    for i, directory in enumerate(directory_list):
        train_np[i] = np.loadtxt(directory)
    
    motor_train_np  = train_np[:,:,:N_MOTOR]
    vision_train_np = train_np[:,:,N_MOTOR:N_MOTOR + N_VISION]
    intero_train_np = train_np[:,:,N_MOTOR + N_VISION:]

    dataset = DataSet(motor_train_np, vision_train_np, intero_train_np, minibatch_size)
    ...
    intero_mse = NormalizeMSE(dataset.intero_dim)
    ...
    for i in range(epo):
        ...
        _top_kl, _prop_kl, _vision_kl, _intero_kl, _kl = 0., 0., 0., 0., 0.
        _top_nll_init_h, _prop_nll_init_h, _vision_nll_init_h, _intero_nll_init_h, _nll_init_h = 0., 0., 0., 0., 0.
        _p_loss, _v_loss, _i_loss, _loss = 0., 0., 0., 0.

        for j in np.random.permutation(range(dataset.n_minibatch)):
            ps, vs, ins, top_kl, prop_kl, vision_kl, intero_kl, top_wkl, prop_wkl, vision_wkl, intero_wkl, top_wnll_init_h, prop_wnll_init_h, vision_wnll_init_h, intero_wnll_init_h, res = model.posterior_forward(i, j)
            p_loss = prop_mse(ps, dataset.motor_minibatch[j])
            v_loss = vision_mse(vs, dataset.vision_minibatch[j])
            i_loss = intero_mse(ins, dataset.intero_minibatch[j])
            
            loss = p_loss + v_loss + i_loss + torch.sum(top_wkl) + torch.sum(prop_wkl) + torch.sum(vision_wkl) + torch.sum(intero_wkl) + torch.sum(top_wnll_init_h) + torch.sum(prop_wnll_init_h) + torch.sum(vision_wnll_init_h) + torch.sum(intero_wnll_init_h)
            ...
            _intero_kl += intero_kl
            _kl += torch.sum(top_kl) + torch.sum(prop_kl) + torch.sum(vision_kl) + torch.sum(intero_kl)
            _p_loss += p_loss.item()
            _v_loss += v_loss.item()
            _i_loss += i_loss.item()
            _loss   += loss.item()
